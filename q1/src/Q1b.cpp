...
// run : g++ -std=c++11 Q1bb.cpp
// ./a.out querynum < network.txt > o.txt
// output file : o.txt
...
void createTestcase(vector <long> samples, long N, vector <long> &Znodes,
						 long &Xi, long &Xj, Node nodes[])
{
	long observedNodes, index;
	...
	observedNodes = rand() % (N/2);  // N/2
	set <long> Z = getObservedNodes(observedNodes, samples);
...
void processQuery(long Q, long N, Node nodes[])
{
	long Xi, Xj;
	...
	for (long q = 1; q <= Q; q++)
	{
		createTestcase(samples, N, Znodes, Xi, Xj, nodes);
		printVector(Znodes);
